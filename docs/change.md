...
新增 CSV 数据导出功能，独立模块实现。

### 新增模块

**文件：** `src/services/export.py`

- `DataExporter` 类：提供 CSV 导出功能
- `export_csv()` 方法：将人员列表导出为 CSV 文件
- `_get_person_info()` 方法：获取人员类型和特有字段描述
...
- 新增菜单项：`10. 导出CSV`
- 新增 `export_csv()` 方法：调用 DataExporter 导出数据

### CSV 输出格式

| 编号 | 姓名 | 性别 | 年龄 | 类型 | 特有字段 |
|------|------|------|------|------|----------|
| T001 | 张三 | 男 | 35 | 教师 | 系部:计算机系, 专业:软件工程, 职称:副教授 |

### 涉及文件

- `src/services/export.py`（新建）
- `src/ui/cmd_ui.py`
...
新增 CSV 数据导入功能，与导出功能配套。

### 新增模块

**文件：** `src/services/csv_import.py`

- `DataImporter` 类：提供 CSV 导入功能
- `TYPE_MAP`：类型名称到类对象的映射
- `FIELD_MAP`：中文字段名到英文字段名的映射
- `import_csv()` 方法：从 CSV 文件导入人员数据
- `_parse_row()` 方法：解析 CSV 行为人员对象
- `_parse_extra_fields()` 方法：解析特有字段字符串
...
- 新增菜单项：`11. 导入CSV`
- 新增 `import_csv()` 方法：调用 DataImporter 导入数据
...
优化导入交互流程，提供用户指引和专门的导入目录。

### 改动内容

1. **新建导入目录**：`data/import/`，专门存放待导入的 CSV 文件
2. **优化交互流程**：
   - 清屏后显示导入指引
   - 提示文件放在 `data/import/` 目录
   - 说明 CSV 格式要求
   - 用户输入文件名（含后缀）
   - 系统拼接路径 `data/import/{filename}`
...
请将CSV文件放入 data/import/ 目录
文件需包含表头：编号,姓名,性别,年龄,类型,特有字段
...
- `data/import/`（新建目录）
- `src/ui/cmd_ui.py`（修改 import_csv 方法）
